<div>
    ...
    @else
        <div class="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
            <div class="max-w-md w-full bg-white rounded-3xl shadow-xl p-10 border border-gray-100 transform transition-all">
                <div class="text-center mb-8">
                    <div class="w-24 h-24 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center mx-auto mb-4 shadow-inner">
                    ...
                    <h2 class="text-3xl font-extrabold text-gray-900">Gerbang Ujian</h2>
                    <p class="mt-2 text-gray-500 text-sm">Ujian ini dilindungi. Silakan masukkan PIN / Token yang diberikan oleh Guru Pengawas.</p>
                </div>
                ...
                @if (session()->has('error_token'))
                    <div class="bg-red-50 border-l-4 border-red-500 text-red-700 p-4 rounded-xl mb-6 flex items-center">
                        ...
                        <p class="font-bold">{{ session('error_token') }}</p>
                    </div>
                @endif

                <div class="space-y-6">
                    <div>
                        <input wire:model="inputToken" type="text" class="w-full text-center text-4xl tracking-widest font-black uppercase bg-gray-50 border border-gray-300 rounded-2xl py-5 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all shadow-inner" placeholder="••••••" maxlength="6" autofocus>
                    </div>
                    <button wire:click="verifikasiToken" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-black py-4 rounded-2xl shadow-lg shadow-blue-200 transition-transform transform hover:scale-[1.02] active:scale-[0.98] uppercase tracking-wider text-lg">
                        Verifikasi & Masuk
                    </button>
                </div>
            </div>
